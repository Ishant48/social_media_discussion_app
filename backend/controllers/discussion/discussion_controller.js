        ...
        if (name && password && email && mobile_number) {
            ...
            if (!nameRegex.test(name)) {
                return res.status(400).json({
                    message: "Name should contain only letters and spaces and be at least 2 characters long.",
                });
            }
            ...
            try {
                let userData = await user_db.createUserDetails(lowerCaseName, passwordHash, email, mobile_number);
                if (userData) {
                ...
                const errorResult = await commonFunction.catchErrorHandling("Error occurred while adding user to the DB.", 400, err);
                next(errorResult);
            }
        } else {
            return res.status(400).json({ message: "All fields are required: name, password, email, mobile number." });
        }
    } catch (err) {
    ...
    try {
        ...
        if (req.body.email === data.email) {
            if (userDoc) {
                ...
                try {
                    let updatedUser = await user_db.updateUserDetails(userDoc._id, updateObj);
                    if (updatedUser) {
                    ...
                    const errorResult = await commonFunction.catchErrorHandling("Error occurred while updating user in the DB.", 400, err);
                    next(errorResult);
                }
            } else {
...
const deleteUser = async (req, res, next) => {
    ...
    try {
        ...
        const errorResult = await commonFunction.catchErrorHandling(`Error occurred while deleting user from the DB.`, 400, err);
        next(errorResult);
...
const searchUser = async (req, res, next) => {
    ...
    try {
        ...
        const errorResult = await commonFunction.catchErrorHandling(`Error occurred while fetching user from the DB.`, 400, err);
        next(errorResult);
...
const getUsersList = async (req, res, next) => {
    ...
    try {
        ...
        const errorResult = await commonFunction.catchErrorHandling(`Error occurred while fetching users data from the DB.`, 400, err);
        next(errorResult);
